...
onMounted(async () => {
  ...
  if (data) {
    for (const row of data) {
      if (row.key === 'registration_open') registrationOpen.value = row.value === true || row.value === 'true'
      if (row.key === 'registration_message') statusMessage.value = row.value || 'Penerimaan Telah Dibuka'
    }
  }
})
</script>
...
<template>
  <section class="relative bg-gradient-to-br from-forest-800 via-forest-600 to-forest-700 text-white overflow-hidden">
    <!-- Background Pattern -->
    ...
    <div class="container mx-auto px-4 py-20 md:py-32 relative">
      <div class="max-w-3xl mx-auto text-center">
        ...
        <p class="text-base text-white/60 mb-10 max-w-xl mx-auto">
          Daftarkan diri Anda sekarang untuk menjadi bagian dari sekolah unggulan.
